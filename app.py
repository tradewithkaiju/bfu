...
def classify_token(symbol):
    try:
        ...
        if m15_trend == h1_trend == h4_trend and m15_trend != 'neutral':
            return m15_trend
        return 'neutral'
    except Exception as e:
...
def run_scanner():
    ...
    if TEST_MODE:
        st.warning(f"⚠️ TEST MODE: Scanning first {TEST_SYMBOLS_COUNT} symbols only")
...
# 🔍 Optional Single Token Test
st.subheader("🔬 Test a Single Token (e.g. BTCUSDT)")
test_symbol = st.text_input("Enter a symbol (optional)", "")
if test_symbol:
    test_symbol = test_symbol.strip().upper()
    valid_symbols = get_futures_symbols()
    if TEST_MODE:
        valid_symbols = requests.get(f"{BASE_URL}/fapi/v1/exchangeInfo").json()['symbols']
        valid_symbols = [s['symbol'] for s in valid_symbols 
                        if s['contractType'] == 'PERPETUAL'
                        and s['quoteAsset'] == 'USDT'
                        and s['status'] == 'TRADING']
    
    if test_symbol not in valid_symbols:
        st.error(f"{test_symbol} is not a valid Binance USDT Perpetual symbol")
    else:
        result = classify_token(test_symbol)
        st.write(f"**Trend for {test_symbol}**: `{result}`")
